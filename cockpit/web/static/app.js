...
(() => {
  ...
  const state = {
    ...
    group: "flat",
    activeJobId: null,
    streamTimers: {},      // job.id → EventSource
    layout: "side",
  };
  ...
  function changelogLink(it) {
    if (it.latest_version)
      return `<button class="text-[12px] font-medium hover:underline" style="color: var(--accent);" data-cl-sw="${it.software}" data-cl-ver="${it.latest_version}">中文</button>`;
  ...
  /* ============================================================
     Changelog Modal
     ============================================================ */
  async function openChangelog(software, version) {
    let v;
    try { v = await API.getChangelog(software, version); }
    catch (e) { toast("err", "讀取 changelog 失敗"); return; }
    ...
    $("#modal-zh").innerHTML = v.changelog_zh ? mdToHtml(v.changelog_zh) : "<p>尚無中文摘要</p>";
  ...
  async function startUpdate(id) {
    ...
    let jobId;
    try { jobId = await API.createJob(it.software, it.machine); }
    catch (e) {
      toast(e.status === 409 ? "warn" : "err",
        e.status === 409 ? `${it.software} 已有進行中的更新` : `建立更新失敗`);
      return;
    }
    it.status = "updating";
    ...
    render(); renderCurrentJob(job); openDrawer(); renderRecentJobs();
    state.streamTimers[job.id] = API.streamJob(job.id,
      (line) => { job.log.push(line); appendLogLine(job, line); },
      (status) => { finishJobReal(job); });
  }

  async function finishJobReal(job) {
    let fresh;
    try { fresh = await API.getJob(job.id); } catch (e) { return; }
    job.status = fresh.status; job.new_version = fresh.new_version;
    try { state.installs = await API.getInstalls(); } catch (e) {}
    if (fresh.status === "success") toast("ok", `${job.software} 已更新${fresh.new_version ? "到 " + fresh.new_version : ""}`);
    else if (fresh.status === "aborted") toast("warn", `已中止 ${job.software} 的更新`);
    else toast("err", `${job.software} 更新失敗`);
    render();
  ...
  async function abortJob(jobId) {
    ...
    if (!job) return;
    try { await API.abortJob(jobId); } catch (e) {}
    toast("warn", `已要求中止 ${job.software}`);
    // the SSE 'done' (status aborted) will drive finishJobReal; close stream defensively
    const es = state.streamTimers[job.id];
    if (es && es.close) { /* keep open until done event arrives */ }
  }
  ...
  document.addEventListener("click", (e) => {
    const cl = e.target.closest("[data-cl-sw]");
    if (cl) return openChangelog(cl.getAttribute("data-cl-sw"), cl.getAttribute("data-cl-ver"));
  ...
  async function boot() {
    ...
    emptyCurrentJob();
    try { state.machines = await API.getMachines(); } catch (e) {}
    initFilters();   // populates the machine dropdown from state.machines
    try { state.installs = await API.getInstalls(); } catch (e) { toast("err", "讀取清單失敗"); }
    try { state.jobs = await API.getJobs(); } catch (e) {}
    renderRecentJobs();
    render();
  }
  boot();
